    ...
    MessageHandler: function (event) {
        return new Promise(async function (resolve, reject) {
            ...
            if (/-All/.test(event.message.text)) {
                if (await Authorize.Owner(event.source.userId)) param[param.length] = 'place="Public"';
                else reject('您沒有權限讀取全部關鍵字回應！');
                return 0;
            }

            if (param.length == 0) reject('缺少參數，參數：-My 顯示自己的回應清單、-Here 顯示此群組內創建的清單、-Public 顯示公共回應清單。');
